...
{
    [TestClass]
    public class CommandHandlersProjectItemsSourceTests
    {
        ...
        [TestMethod]
        public void CommandHandlersProjectItemSource_should_get_command_handlers_project_from_solution()
        {
            CommandHandlersProjectItemsSource inst = new CommandHandlersProjectItemsSource(this.MockSolutionProvider);
            ...
            Assert.IsNotNull(pm);
            Assert.IsTrue(pm.Name.EndsWith("business", StringComparison.InvariantCultureIgnoreCase));
        }
    }
}
