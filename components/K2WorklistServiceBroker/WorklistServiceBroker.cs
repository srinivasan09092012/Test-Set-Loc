...
namespace SourceCode.SmartObjects.Services.WorklistService
{
    ...
    public class WorklistServiceBroker : ServiceAssemblyBase
    {
        ...
        //private ServicePackage sp = new ServicePackage();
        ...
        public override string GetConfigSection()
        {
            ...
            // HP Added for Managed User Support
            base.Service.ServiceConfiguration.Add("Connection String", true, "Integrated=True;IsPrimaryLogin=False;Authenticate=True;EncryptedPassword=False;Host=localhost;Port=4252");
            base.Service.ServiceConfiguration.Add("Impersonate Connection String", true, "Integrated=True;IsPrimaryLogin=True;Authenticate=True;EncryptedPassword=True;Host=localhost;Port=4252;WindowsDomain=MyDomain;UserID=SvcAccount;Password=password");
        ...
        public override string DescribeSchema()
        {
            ...
            base.Service.ServiceObjects.Create(bwi.DescribeServiceObject());
            base.Service.ServiceObjects.Create(dwi.DescribeServiceObject());
            base.Service.ServiceObjects.Create(wla.DescribeServiceObject());
                            ...
                            case "basicworklistitem":
                                {
                                    ...
                                    // HP Added for Managed User Support
                                    if (methodName == "getmanageduserworklistitems")
                                    {
                                        if ((properties.ContainsKey("UserName") && (properties["UserName"].ToString() != string.Empty)))
                                        {
                                            result = worklist.GetManagedUserWorklistItems(properties, parameters, properties["UserName"].ToString());
                                        }
                                        else
                                            throw new ArgumentException("Input value cannot be null or empty", "userName");
                                    }
                            ...
                            case "worklistitemaction":
                                {
                                    ...
                                    // HP Added for Managed User Support
                                    if (methodName == "actionmanageduserworklistitem")
                                        worklistaction.ActionManagedUserWorklistItem(Convert.ToString(parameters["ManagedUserName"]), Convert.ToString(properties["SerialNumber"]), Convert.ToString(properties["ActionName"]));

                                    // HP Added for Managed User Support
                                    if (methodName == "releaseworklistitem")
                                        worklistaction.ReleaseWorklistItem(Convert.ToString(properties["SerialNumber"]));

                                    // HP Added for Managed User Support
                                    if (methodName == "releasemanageduserworklistitem")
                                        worklistaction.ReleaseManagedUserWorklistItem(Convert.ToString(parameters["ManagedUserName"]), Convert.ToString(properties["SerialNumber"]));
                                    break;
                                }
                        }
                    }
